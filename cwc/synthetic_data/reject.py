...
def create_reject_data(X, proportion, method, pca=False, pca_components=0,
    ...
    Args:
        ...
        pca_components (int): e.g. 2 Number of dimensions of the hyper-shape in the
            lower dimensionality space (this option and pca_variance are
            mutually exclusive if both options are set at the same time the
            function will raise an Exception).
        pca_variance (float): e.g. 0.99 Proportion of variance (0.0,1.0) to keep in the
            lower dimensionality space (this option and pca_components are
            mutually exclusive if both options are set at the same time the
            function will raise an Exception).
    ...
    if pca == True:
        if pca_components and pca_variance:
            raise Exeption('Options pca_components and pca_variance are'
                           'mutually exclusive')
        elif pca_components:
            ...
            raise Exeption('If PCA is selected pca_components or pca_variance '
    ...
    if method == 'uniform_hcube':
        r = hypercube_distribution(num_reject, dimensions)
        if hshape_cov:
            r *= numpy.sqrt(hshape_cov/(numpy.mean(r**2)))
    elif method == 'uniform_hsphere':
        if hshape_prop_in and hshape_cov:
            raise Exeption('Options hshape_prop_in and hshape_cov are'
                           'mutually exclusive')
